...
class Forecaster(L.LightningModule, ABC):
    def __init__(
        ...
        if linear_window:
            self.linear_model = linear_model.LinearModel(
        ...
        if use_revin:
            ...
            self.revin = revin.RevIN(num_features=d_yc)
        else:
            self.revin = lambda x, *args, **kwargs: x

        self.use_seasonal_decomp = use_seasonal_decomp
        if use_seasonal_decomp:
            self.seasonal_decomp = revin.SeriesDecomposition(kernel_size=25)
        else:
    ...
    def _compute_stats(
        ...
        stats = {
            "mape": eval_stats.mape(scaled_true, scaled_pred) / adj,
            "mae": eval_stats.mae(scaled_true, scaled_pred) / adj,
            "mse": eval_stats.mse(scaled_true, scaled_pred) / adj,
            "smape": eval_stats.smape(scaled_true, scaled_pred) / adj,
            "norm_mae": eval_stats.mae(true, pred) / adj,
            "norm_mse": eval_stats.mse(true, pred) / adj,
        }
        return stats
    ...
    def on_train_batch_end(self, outs, batch, batch_idx):
        self._log_stats("train", outs)
        return {"loss": outs["loss"].mean()}

    def on_validation_batch_end(self, outs, batch, batch_idx):
        self._log_stats("val", outs)
        return outs

    def on_test_batch_end(self, outs, batch, batch_idx):
